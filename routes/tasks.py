from flask import Blueprint, jsonify, request
from urllib.parse import unquote
import sqlite3
from database.db import get_connection
from database.task_queries import delete_task_from_db, get_punishment, mark_task_failed, get_tasks
from database.user_queries import get_user
from helpers.task_helpers import (
    get_character_stats, update_character_stats,
    get_task_by_name, mark_task_completed,
    insert_task, update_task_in_db,
    calculate_xp_and_level_up, calculate_xp_and_level_down
)

tasks_bp = Blueprint('tasks', __name__)

# ----------------- Complete Task -----------------
@tasks_bp.route("/completed_task/<name>", methods=["POST"])
def completed_task(name):
    name = unquote(name)
    conn = sqlite3.connect('data.db')
    conn.row_factory = sqlite3.Row

    task = get_task_by_name(conn, name)
    if not task:
        conn.close()
        return jsonify({"error": "Task not found"}), 404

    stats = get_character_stats(conn)
    new_xp, new_level, new_xp_to_next_level = calculate_xp_and_level_up(
        stats['xp'], task['xp_reward'], stats['level'], stats['xp_to_next_level']
    )
    new_coins = stats['coins'] + task['coin_reward']

    update_character_stats(conn, new_level, new_coins, new_xp, new_xp_to_next_level)
    mark_task_completed(conn, name, True)
    conn.close()

    return jsonify({
        "task": name,
        "xp_gained": task['xp_reward'],
        "coins_gained": task['coin_reward'],
        "new_level": new_level,
        "new_xp": new_xp
    })

# ----------------- Uncomplete Task -----------------
@tasks_bp.route("/uncomplete_task/<name>", methods=["POST"])
def uncomplete_task(name):
    name = unquote(name)
    conn = sqlite3.connect('data.db')
    conn.row_factory = sqlite3.Row

    task = get_task_by_name(conn, name)
    if not task:
        conn.close()
        return jsonify({"error": "Task not found"}), 404

    stats = get_character_stats(conn)
    new_xp, new_level, new_xp_to_next_level = calculate_xp_and_level_down(
        stats['xp'], task['xp_reward'], stats['level'], stats['xp_to_next_level']
    )
    new_coins = max(0, stats['coins'] - task['coin_reward'])

    update_character_stats(conn, new_level, new_coins, new_xp, new_xp_to_next_level)
    mark_task_completed(conn, name, False)
    conn.close()

    return jsonify({
        "task": name,
        "xp_removed": task['xp_reward'],
        "coins_removed": task['coin_reward'],
        "new_level": new_level,
        "new_xp": new_xp,
        "new_coins": new_coins
    })

# ----------------- Add Task -----------------
@tasks_bp.route('/add_task', methods=['POST'])
def add_task():
    user = get_user()

    task_name = request.form.get('task_name')
    coin_reward = request.form.get('coin_reward')
    xp_reward = request.form.get('xp_reward')
    start_time = request.form.get('start_time') or None
    end_time = request.form.get('end_time') or None
    repeat_days = request.form.getlist('repeat_days') or []
    repeat_days_str = ",".join(repeat_days) if repeat_days else None

    conn = sqlite3.connect('data.db')
    task_id = insert_task(conn, user["id"], task_name, coin_reward, xp_reward, start_time, end_time, repeat_days_str)
    conn.close()

    task_dict = {
        "task_id": task_id,
        "task_name": task_name,
        "coin_reward": coin_reward,
        "xp_reward": xp_reward,
        "start_time": start_time,
        "end_time": end_time,
        "repeat_days": repeat_days
    }

    return jsonify({"status": "success", "message": "Task added!", "task": task_dict})

# ----------------- Add Punishment as Task -----------------
@tasks_bp.route('/add_punishment_as_task', methods=['POST'])
def add_punishment_as_task():
    punishment = request.get_json()
    task_name = punishment['task_name']
    coin_reward = punishment['coin_reward']
    xp_reward = punishment['xp_reward']

    conn = get_connection()
    task_id = insert_task(conn, user["id"], task_name, coin_reward, xp_reward)
    conn.close()

    punishment_dict = {
        "task_id": task_id,
        "task_name": task_name,
        "coin_reward": coin_reward,
        "xp_reward": xp_reward,
        "start_time": None,
        "end_time": None,
        "repeat_days": None
    }

    return jsonify({"status": "success", "message": "punishment added!", "punishment": punishment_dict})

# ----------------- Update Task -----------------
@tasks_bp.route("/update_task/<task_id>", methods=['POST'])
def update_task(task_id):
    user = get_user()
    
    task_name = request.form.get('task_name')
    coin_reward = request.form.get('coin_reward')
    xp_reward = request.form.get('xp_reward')
    start_time = request.form.get('start_time') or None
    end_time = request.form.get('end_time') or None
    days = request.form.getlist('repeat_days') or []
    repeat_days_str = ",".join(days) if days else None

    conn = sqlite3.connect('data.db')
    update_task_in_db(conn, user["id"], task_id, task_name, coin_reward, xp_reward, start_time, end_time, repeat_days_str)
    conn.close()

    task_dict = {
        "task_id": task_id,
        "task_name": task_name,
        "coin_reward": coin_reward,
        "xp_reward": xp_reward,
        "start_time": start_time,
        "end_time": end_time,
        "repeat_days": repeat_days_str
    }

    return jsonify({"status": "ok", "task": task_dict})

# ----------------- Delete Task -----------------
@tasks_bp.route('/delete_task/<task_id>', methods=['POST'])
def delete_task(task_id):
    delete_task_from_db(task_id)
    return jsonify({"success": True})

# ----------------- Mark Task Failed -----------------
@tasks_bp.route("/mark_task_failed/<task_id>", methods=['POST'])
def mark_task_failed_route(task_id):
    mark_task_failed(task_id)
    return jsonify({"status": "ok", "message": f"Task {task_id} marked as failed"})

# ----------------- Get Task Details -----------------
@tasks_bp.route("/getTaskDetails/<task_id>", methods=['POST'])
def getTaskDetails(task_id):
    tasks = get_tasks()
    wanted_task = next((t for t in tasks if int(t['task_id']) == int(task_id)), None)
    if wanted_task:
        return jsonify({"status": "ok", "task": wanted_task})
    return jsonify({"status": "not found"})

# ----------------- Get Punishment -----------------
@tasks_bp.route("/getPunishmint/<punishmint_id>", methods=['POST'])
def getPunishmint(punishmint_id):
    punishment = get_punishment(punishmint_id)
    return jsonify(punishment)
